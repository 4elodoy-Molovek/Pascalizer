...
static void ANALYSIS_MACHINE_CHECK(const std::vector<Token>& tokenStream, const std::vector<std::string>& expectedCodeNotation)
{
	...
	ASSERT_EQ(AnalysisStatus::FINISHED, analysisMachine->GetStatus());
	ASSERT_EQ(expectedCodeNotation.size(), code.size());
	...
	while (instructionPointer)
	{
		...
		else if (instructionPointer->pUp)
		{
...
static void ANALYSIS_MACHINE_ERROR_CHECK(const std::vector<Token>& tokenStream)
{
	...
	EXPECT_TRUE(analysisMachine->GetStatus() == ERROR || analysisMachine->GetStatus() == ONGOING);
}
...
TEST(AnalysisMachine, Can_Analyze_Int_Const_Declaration)
{
	...
	std::vector<std::string> expectedCodeNotation =
	{
		"IProgram(test)", "-IConstBlock()", "--IDeclareConst(Int, 3)", "-IMainBlock()"
	};
...
TEST(AnalysisMachine, Can_Analyze_Double_Const_Declaration)
{
	...
	std::vector<std::string> expectedCodeNotation =
	{
		"IProgram(test)", "-IConstBlock()", "--IDeclareConst(Doub, 3.140000)", "-IMainBlock()"
	};
...
TEST(AnalysisMachine, Can_Analyze_String_Const_Declaration)
{
	...
	std::vector<std::string> expectedCodeNotation =
	{
		"IProgram(test)", "-IConstBlock()", "--IDeclareConst(Str, some thing like a name, idk)", "-IMainBlock()"
	};
...
TEST(AnalysisMachine, Can_Analyze_Multiple_Const_Declarations)
{
	...
	std::vector<std::string> expectedCodeNotation =
	{
		...
		"--IDeclareConst(Str, some thing like a name, idk)", "--IDeclareConst(Doub, 3.140000)", "--IDeclareConst(Int, 3)", "-IMainBlock()"
	};
...
TEST(AnalysisMachine, Can_Analyze_Double_Variable_Declaration)
{
	...
	std::vector<std::string> expectedCodeNotation =
	{
		"IProgram(test)", "-IVarBlock()", "--IDeclareVar(DOUBLE, Doub)", "-IMainBlock()"
	};
...
TEST(AnalysisMachine, Can_Analyze_String_Variable_Declaration)
{
	...
	std::vector<std::string> expectedCodeNotation =
	{
		"IProgram(test)", "-IVarBlock()", "--IDeclareVar(STRING, Str)", "-IMainBlock()"
	};
...
TEST(AnalysisMachine, Can_Analyze_Multiple_Variable_Declarations)
{
	...
	std::vector<std::string> expectedCodeNotation =
	{
		"IProgram(test)", "-IVarBlock()", "--IDeclareVar(STRING, Str)", "--IDeclareVar(DOUBLE, Doub)", "--IDeclareVar(INT, Int)", "-IMainBlock()"
	};
...
TEST(AnalysisMachine, Can_Analyze_Single_Line_Multi_Int_Variable_Declaration)
{
	...
	std::vector<std::string> expectedCodeNotation =
	{
		"IProgram(test)", "-IVarBlock()", "--IDeclareVar(INT, Int_1)", "--IDeclareVar(INT, Int_2)", "-IMainBlock()"
	};
...
TEST(AnalysisMachine, Can_Analyze_Single_Line_Multi_Double_Variable_Declaration)
{
	...
	std::vector<std::string> expectedCodeNotation =
	{
		"IProgram(test)", "-IVarBlock()", "--IDeclareVar(DOUBLE, Doub_1)", "--IDeclareVar(DOUBLE, Doub_2)", "-IMainBlock()"
	};
...
TEST(AnalysisMachine, Can_Analyze_Single_Line_Multi_String_Variable_Declaration)
{
	...
	std::vector<std::string> expectedCodeNotation =
	{
		"IProgram(test)", "-IVarBlock()", "--IDeclareVar(STRING, Str_1)", "--IDeclareVar(STRING, Str_2)", "-IMainBlock()"
	};
...
TEST(AnalysisMachine, Can_Handle_Double_Variable_Assignment)
{
	std::vector<Token> tokenStream =
	{
		...
		{NAME, "Doub"}, {COLON, ":"}, {NAME, "double"}, {END_LINE, ";"},
		{BEGIN, "begin"},
		{NAME, "Doub"}, {ASSIGN_OPERATOR, ":="}, {VALUE_INT, "3"}, {END_LINE, ";"},
		{END, "end"},
		{PROGRAMM_END, "."}
	};
...
TEST(AnalysisMachine, Can_Handle_String_Variable_Assignment)
{
	std::vector<Token> tokenStream =
	{
		...
		{NAME, "Str"}, {COLON, ":"}, {NAME, "string"}, {END_LINE, ";"},
		{BEGIN, "begin"},
		{NAME, "Str"}, {ASSIGN_OPERATOR, ":="}, {VALUE_INT, "3"}, {END_LINE, ";"},
		{END, "end"},
		{PROGRAMM_END, "."}
	};
...
TEST(AnalysisMachine, Can_Handle_Function_Calls)
{
	...
	std::vector<std::string> expectedCodeNotation =
	{
		"IProgram(test)", "-IMainBlock()", "--IWrite()"
	};
...
TEST(AnalysisMachine, Can_Handle_If_Statements)
{
	...
	std::vector<std::string> expectedCodeNotation =
	{
		"IProgram(test)", "-IMainBlock()", "--IIf()", "---IWrite()"
	};
...
TEST(AnalysisMachine, Can_Handle_While_Statements)
{
	std::vector<Token> tokenStream =
	{
		{PROGRAM, "program"}, {NAME, "test"}, {END_LINE, ";"},
...
TEST(AnalysisMachine, Can_Handle_Nested_Code_Blocks_With_While)
{
	...
	{
		"IProgram(test)", "-IMainBlock()",
		"--IWhile()", "---IIf()","----IWrite()", "---IElse()", "----IWrite()", "--IWrite()"
	};
...
TEST(AnalysisMachine, Can_Handle_Expressions_In_Variable_Assignment)
{
	...
	// Standard check
	intructions = ANALYSIS_MACHINE_INSTRUCTION_VECTOR(tokenStream);

	assignVar = dynamic_cast<IAssignVar*>(intructions[4].get());
...
TEST(AnalysisMachine, Can_Handle_Expressions_In_Function_Calls)
{
	...
	IWrite* writeInstr = dynamic_cast<IWrite*>(intructions[4].get());
	EXPECT_EQ("3 4 2 * + , ", writeInstr->GetExpressionNotation());
	...
	tokenStream =
	{
		...
		{MATH_OPERATOR, "="}, {VALUE_INT, "11"}, {MATH_OPERATOR, "and"}, {VALUE_INT, "10"}, {MATH_OPERATOR, "div"}, {VALUE_INT, "2"}, {MATH_OPERATOR, "="}, {VALUE_INT, "5"}, {BRACKET_CLOSE, ")"}, {END_LINE, ";"},
		{END, "end"},
		{PROGRAMM_END, "."}
	};
	...
	writeInstr = dynamic_cast<IWrite*>(intructions[4].get());
	EXPECT_EQ("3 4 2 * + 11 = 10 2 div 5 = and , ", writeInstr->GetExpressionNotation());
}
